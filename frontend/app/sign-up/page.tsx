...
export default function SignUp() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (data?.requireEmailVerification) setStep('verify')
    else if (data?.accessToken) router.push('/dashboard')
  }
      ...
      <div className="w-full max-w-sm relative animate-fade-in">
        ...
        <div className="bg-slate-900/60 backdrop-blur border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
          <form onSubmit={handleVerify} className="space-y-4">
            <input
              type="text" placeholder="000000" value={otp} onChange={e => setOtp(e.target.value)}
              maxLength={6} required
              className="w-full px-4 py-4 bg-slate-800/80 text-white rounded-xl border border-slate-600/50 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 text-center text-3xl tracking-[0.5em] font-mono transition-all"
            />
            {error && (
